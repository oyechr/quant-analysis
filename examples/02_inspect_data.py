...
def inspect_data(ticker: str, period: str = "1y", use_cache: bool = True):
    ...
    print(f"Date range: {format_date(data.index[0], 'readable')} to {format_date(data.index[-1], 'readable')}")
    ...
    print("\n5. Price Information")
    print("-" * 40)
    print(f"Highest Close: ${data['Close'].max():.2f} on {format_date(data['Close'].idxmax(), 'readable')}")
    print(f"Lowest Close:  ${data['Close'].min():.2f} on {format_date(data['Close'].idxmin(), 'readable')}")
    ...
    print(f"Max Volume:     {data['Volume'].max():,.0f} on {format_date(data['Volume'].idxmax(), 'readable')}")
    print(f"Min Volume:     {data['Volume'].min():,.0f} on {format_date(data['Volume'].idxmin(), 'readable')}")
