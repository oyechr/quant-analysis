...
def main():
    ...
    custom_data = fetcher.fetch_ticker(
        "MSFT",
        start="2024-01-01",
        end="2024-12-31",
        interval="1d"
    ...
    print(f"Price change: ${custom_data['Close'].iloc[0]:.2f} -> ${custom_data['Close'].iloc[-1]:.2f}")
    print(f"Return: {((custom_data['Close'].iloc[-1] / custom_data['Close'].iloc[0]) - 1) * 100:.2f}%")
